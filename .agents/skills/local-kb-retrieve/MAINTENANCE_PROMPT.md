...
- Run generalization review before creating or changing card surfaces. Classify evidence as `project-local`, `skill-specific`, `single-project-generalizable`, `cross-project-general`, or `insufficient-evidence`.
- Do not force Skill/plugin/tool lessons into capability-independent rules. If the evidence depends on a Codex Skill, plugin, connector, or tool capability, keep that capability boundary visible in the route, wording, or provenance.
...
8. Preserve and inspect provenance for each observation when available: timestamp, agent name, thread reference, project reference, and workspace root. This metadata matters when deciding whether a lesson is one-off, project-local, or worth re-review by a similar agent flow later.
   ...
   Generalization rule: same-project repetition strengthens chronology, not automatic universality. Cross-project evidence strengthens generality. Skill-specific evidence is a valid bounded outcome: keep the Skill/plugin/tool context when that capability controls the lesson.
...
`python .agents/skills/local-kb-retrieve/scripts/kb_consolidate.py --json --run-id <run_id>-semantic --apply-mode semantic-review --semantic-review-plan kb/history/consolidation/<run_id>-semantic/semantic_review_plan.yaml`
   The plan must use decisions `keep`, `rewrite`, `adjust-confidence`, `promote`, `demote`, or `deprecate` for automatic apply. `split` and `merge` may be recorded as proposal-only until they are represented as concrete safe rewrites. Each applied decision must set `apply: true`, cite current action `evidence_event_ids`, include `rationale`, `risk`, `utility_assessment`, `scope_assessment`, `expected_retrieval_effect`, and `rollback_note`, and respect the trusted-card budget of 3. Use `utility_assessment.judgment: useful` for `keep`, `rewrite`, `adjust-confidence`, or `promote`; use a non-useful judgment such as `low-utility`, `obsolete`, `misleading`, `unclear`, or `insufficient-evidence` for `demote` or `deprecate`. Use `scope_assessment.scope: skill-specific` when the rule depends on a Skill, plugin, connector, or tool capability rather than on the project where it was observed.
