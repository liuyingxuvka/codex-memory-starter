...
- When Skill-use observations imply a Skill prompt or workflow should change, keep the card/candidate work in Sleep but surface the Skill change as proposal-only `review-code-change` evidence for Architect. Sleep must not directly rewrite Skill files.
